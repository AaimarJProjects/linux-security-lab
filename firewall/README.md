...
| Port 23/tcp - DENY | Drops traffic going to Port 23, telnet, an unsecure version of ssh because any data that passes through it is sent in plain text.| This is a deliberate test configuration, I don't use telnet, I added this rule to see what happens when I have an explicit deny rule but in a production environment I would not add a rule for this port as any traffic to it would be denied by default and an attacker would not know if there is anything on that port. |
